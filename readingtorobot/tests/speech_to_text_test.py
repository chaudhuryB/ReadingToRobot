...
class SpeechRecoMock(VoiceRecognition):
    """Mock class used to test the speech recognition module."""
    ...
    def _process_text(self, text):
        ...
        self._logger.debug("\033[93mRecognized: {}\033[0m".format(text))
        op = self._book.evaluate_static_sentence_validity(text)
        if op is not None:
            self._logger.debug("\033[93mEvaluate text thinks: {}\033[0m".format(op))
...
def main():
    ...
    parser.add_argument(
        ...
        "--play",
        type=str,
        default=resource_file("the_teeny_tree_literal.txt"),
